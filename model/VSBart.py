import math, random, copy, collections
...
def _expand_attn(attn: torch.Tensor, dtype: torch.dtype, tgt_len: Optional[int] = None, head_num: Optional[int] = None):
    """
    Expands attention_mask from `[bsz, seq_len]` to `[bsz, 1, tgt_seq_len, src_seq_len]`.
    """
    bsz, src_len = attn.size() # seq_len
    tgt_len = tgt_len if tgt_len is not None else src_len
    head_num = head_num if head_num is not None else 12

    expanded_attn = attn[:, None, None, :].expand(bsz, head_num, tgt_len, src_len).to(dtype).contiguous()

    return expanded_attn
...
class VideoBartEncoder(BartPretrainedModel):
    ...
    def __init__(self, config: BartConfig, embed_tokens: Optional[nn.Embedding] = None, video_ff: Optional[nn.Linear]=None, d_feature=None):
        ...
        self.embed_patches = PatchEmbeddings()
        if video_ff is not None:
            self.video_ff = video_ff
        else:
            self.video_ff = nn.Linear(d_feature, embed_dim)

        self.embed_positions = BartLearnedPositionalEmbedding(
            config.max_position_embeddings,
            embed_dim,
        )
        self.embed_session = nn.Embedding(31, embed_dim, 0)
        self.embed_scene = nn.Embedding(31, embed_dim, 0)
    ...
    def forward(
        ...
        if input_ids is not None and inputs_embeds is not None:
            ...
            if video_ids is not None:
                if type(video_ids) is list:
                    img_len_lst = [fea.size(0) for fea in video_ids]
                    video_ids = torch.cat(video_ids, dim=0)
                    patch_embeds = self.embed_patches(video_ids) # (n, 9)
                    patch_lst = []
                    start = 0
                    for length in img_len_lst:
                        clip_lst = []
                        for ii in range(start, start+length):
                            clip_lst.append(patch_embeds[ii])
                            clip_lst.append(patch_embeds[ii].mean(0).unsqueeze(0))
                        clip_patch_embed = torch.cat(clip_lst, dim=0)
                        patch_lst.append(clip_patch_embed)
                        start += length
                    video_embeds = self.padding(patch_lst, 0)
                    # video_attention_mask = torch.sum(video_embeds, dim=2) != 0
                    # attention_mask = torch.cat([video_attention_mask, attention_mask], dim=1)
                else:
        ...
        if session is not None:
            session_embeds = self.embed_session(session)
            if scene is not None:
                scene_embeds = self.embed_scene(scene)
                session_embeds = torch.cat([scene_embeds, session_embeds], dim=1)
        else:
            session_embeds = 0
        ...
        hidden_states = inputs_embeds + embed_pos + token_type_embeds + session_embeds
    ...
    def padding(self, seq, pad_token, limit=500, embed=True):
        max_len = max([i.size(0) for i in seq])
        max_len = min(max_len, limit)
        if embed:
            result = (torch.ones((len(seq), max_len, seq[0].size(-1)), device=seq[0].device).float() * pad_token)
        else:
            result = (torch.ones((len(seq), max_len), device=seq[0].device).long() * pad_token)
...
class VideoBartDecoder(BartPretrainedModel):
    ...
    def __init__(self, config: BartConfig, embed_tokens: Optional[nn.Embedding] = None, video_ff: Optional[nn.Linear] = None, d_feature=None):
        ...
        if video_ff is not None:
            ...
            self.video_ff = nn.Linear(d_feature, config.d_model)
...
class VSTARBartModel(BartPretrainedModel):
    def __init__(self, config, d_feature):
        super().__init__(config)
        padding_idx, vocab_size = config.pad_token_id, config.vocab_size
        self.shared = nn.Embedding(vocab_size, config.d_model, padding_idx)
        self.shared_videoff = nn.Linear(d_feature, config.d_model)
        self.encoder = VideoBartEncoder(config, self.shared, self.shared_videoff, d_feature=d_feature)
        self.decoder = VideoBartDecoder(config, self.shared, self.shared_videoff, d_feature=d_feature)
...
class VSTARBARTGenerationModel(BartPretrainedModel):
    def __init__(self, config: BartConfig, d_feature):
        super().__init__(config)
        self.model = VSTARBartModel(config, d_feature=d_feature)
        # video embedding
        ...
        self.seg_head = nn.Linear(config.d_model, 2, bias=False) # True/False
        self.video_inverse_ff = nn.Linear(config.d_model, d_feature)
    ...
    def forward(
        ...
        # if label is not None(Training), automatically generate decoder input for training
        ...
        hidden_states = bart_outputs[0] # [B, seq_len, embed_size]
        lm_logits = self.lm_head(hidden_states)  # [B, seq_len, vocab_size]
        
        loss = None

        if seg_label is not None:
            encoder_hidden_states = bart_outputs[2]
            seg_logits = self.seg_head(encoder_hidden_states)
            loss_seg_fct = CrossEntropyLoss(ignore_index=-1)
            loss = loss_seg_fct(seg_logits.view(-1, seg_logits.size(-1)), seg_label.view(-1))
        ...
        return Seq2SeqLMOutput(
            ...
            decoder_hidden_states=bart_outputs.decoder_hidden_states,
            decoder_attentions=bart_outputs.decoder_attentions,
            cross_attentions=bart_outputs.cross_attentions,
