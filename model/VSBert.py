...
class BertVisEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        super().__init__()
        self.vis_embeddings = nn.Linear(1000, config.hidden_size)
    ...
    def forward(
        self, fea_ids=None, input_ids=None, token_type_ids=None, position_ids=None, inputs_embeds=None, past_key_values_length=0
    ):
        ...
        if inputs_embeds is None:
            inputs_embeds = self.vis_embeddings(fea_ids)
        word_embeds = self.word_embeddings(input_ids)
        inputs_embeds = torch.cat([inputs_embeds, word_embeds], dim=1)
...
class BertEmbeddings(nn.Module):
    ...
    def forward(
        ...
        fea_embeds = self.vis_embeddings(fea_ids)
        inputs_embeds = torch.cat([fea_embeds, inputs_embeds], dim=1)
        token_type_embeddings = self.token_type_embeddings(token_type_ids)
...
class BertSegPooler(nn.Module):
    ...
    def forward(self, hidden_states, seg_indexs):
        ...
        # cidx = hidden_states.size(1) // 2 - 1
        # return hidden_states[:, cidx, :]
        ...
        for ii in range(len(seg_indexs)):
            ...
            # seg_mean = []
            # seg_index = seg_indexs[ii]
            # start = 0
            # for end in seg_index:
            #     seg_mean.append(hidden_states[ii, start:end].mean(dim=0)) # [768]
            #     start = end
            # seg_mean = torch.stack(seg_mean, dim=0) # [seq, 768]
            # seg_mean_lst.append(seg_mean)
...
class BertForSegClassification(BertPreTrainedModel):
    ...
    def forward(
        ...
        if labels is not None:
            ...
            # if seg_indexs is not None:
            #     label_lst = []
            #     for i in range(labels.size(0)):
            #         label_lst.append(labels[i][labels[i]!=-1])
