...
class SklearnAPMetric(torchmetrics.Metric):
    ...
    def compute(self) -> torch.Tensor:
        # self.prob = self.prob[0]
        # self.gts = self.gts[0]
        prob = torch.cat(self.prob).cpu().numpy()
...
class MovieNetMetric(torchmetrics.Metric):
    ...
    def compute(self) -> torch.Tensor:
        ...
        # self.vidx_data = self.vidx_data[0]
        # self.sid_data = self.sid_data[0]
...
class DTSMetric(torchmetrics.Metric):
    def __init__(self):
        super().__init__(compute_on_step=False, dist_sync_on_step=True)
        self.add_state("vidx_data", default=[], dist_reduce_fx="cat")
        self.add_state("sid_data", default=[], dist_reduce_fx="cat")
    ...
    def compute(self) -> torch.Tensor:
        ...
        # recall = self._compute_exact_recall(result)
        # recall_at_second = self._compute_recall_at_second(result)
        wd, pk, f1 = self._compute_session_eval(result)
    ...
    def _compute_windown_diff(self, result):
        wd = []
        for _, result_dict_one in result.items():
            preds, gts = [], []
            for _, item in result_dict_one.items():
                ...
                preds.append(pred)
                gts.append(gt)
            wd_clip = self._session_eval_wf(preds, gts)
            wd.append(wd_clip)
        wd = np.mean(wd)
        pt_wd = self.pred_data[0].new_tensor(wd, dtype=torch.float)
        del wd, wd_clip, pred, gt, preds, gts
        return pt_wd

    def _session_eval_wf(self, pred, ref):
        seg_p = []
        prev = 0
        ...
        for i in range(len(ref)):
            if ref[i]:
                seg_r.append(i-prev)
                prev = i
        seg_r.append(len(ref)+1-prev)

        score_wd = segeval.window_diff(seg_p, seg_r)
        # score_pk = segeval.pk(seg_p, seg_r)
        # score_f1 = f1_score(ref, pred, labels=[0,1], average='macro', zero_division=1)
        return score_wd

    def _compute_pk(self, result):
        pk = []
        for _, result_dict_one in result.items():
            preds, gts = [], []
            for _, item in result_dict_one.items():
                ...
                preds.append(pred)
                gts.append(gt)
            pk_clip = self._session_eval_pk(preds, gts)
            pk.append(pk_clip)
        pk = np.mean(pk)
        pt_pk = self.pred_data[0].new_tensor(pk, dtype=torch.float)
        del pk, pk_clip, pred, gt, preds, gts
        return pt_pk

    def _session_eval_pk(self, pred, ref):
        seg_p = []
        prev = 0
        for i in range(len(pred)):
            if pred[i]:
    ...
    def _compute_session_eval(self, result):
        wd, pk, f1 = [], [], []
        for _, result_dict_one in result.items():
            preds, gts = [], []
            for _, item in result_dict_one.items():
                ...
                preds.append(pred)
                gts.append(gt)
            wd_clip, pk_clip, f1_clip = self._session_eval(preds, gts)
            wd.append(wd_clip)
            pk.append(pk_clip)
            f1.append(f1_clip)
        wd = np.mean(wd)
        ...
        pt_wd = self.pred_data[0].new_tensor(wd, dtype=torch.float)
        pt_pk = self.pred_data[0].new_tensor(pk, dtype=torch.float)
        pt_f1 = self.pred_data[0].new_tensor(f1, dtype=torch.float)
        del wd, pk, f1, wd_clip, pk_clip, f1_clip, pred, gt, preds, gts
        return pt_wd, pt_pk, pt_f1

    def _session_eval(self, pred, ref):
        seg_p = []
        prev = 0
        for i in range(len(pred)):
            if pred[i]:
        ...
        score_pk = segeval.pk(seg_p, seg_r)
        score_f1 = f1_score(ref, pred, labels=[0,1], average='macro', zero_division=1)
        return score_wd, score_pk, score_f1


import segeval
from sklearn.metrics import f1_score
def session_eval(pred, ref):
    seg_p = []
    prev = 0
    for i in range(len(pred)):
        if pred[i]:
